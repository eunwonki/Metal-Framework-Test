...
class CustomMesh: Mesh {
    ...
    init(modelName: String) {
        createVertices(modelName: modelName)
        createBuffers()
    }
    
    func createVertices() {}
    
    func createVertices(modelName: String) {
        guard let assetURL = Bundle.main.url(forResource: modelName, withExtension: "obj") else {
            fatalError("Asset \(modelName) does not exist.")
        }
        let mdlMesh = MDLAsset(url: assetURL).object(at: 0) as! MDLMesh
        
        let vertexData = mdlMesh.vertexAttributeData(
            forAttributeNamed: MDLVertexAttributePosition,
            as: .float3)?
            .dataStart.bindMemory(to: float3.self, capacity: mdlMesh.vertexCount)
        let normalData = mdlMesh.vertexAttributeData(
            forAttributeNamed: MDLVertexAttributePosition,
            as: .float3)?
            .dataStart.bindMemory(to: float3.self, capacity: mdlMesh.vertexCount)
        
        for i in 0..<mdlMesh.vertexCount {
            addVertex(position: vertexData![i], normal: normalData![i])
        }
    }
